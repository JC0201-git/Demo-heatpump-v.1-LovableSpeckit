...
- [ ] T033 [P] 更新現有全域狀態模組：在 `app-state.tsx` 中為 `RoleContext` 加入 localStorage 持久化（key: `x-role`，預設值 `operator`）；保留現有角色切換 UI；移除 `setInterval` mock 資料定時更新邏輯（改由 React Query 負責）（更新 `frontend/src/lib/app-state.tsx`）
